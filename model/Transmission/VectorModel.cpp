...
void VectorModel::calculateEIR(Host::Human &human, double ageYears, vector<double> &EIR_i, vector<double> &EIR_l) const
{
    ...
    if(simulationMode == transientEIRknown)
    {
        ...
        for (size_t i = 0; i < speciesIndex.size(); ++i)
        {
            ...
            mon::record(mon::measure::innoculationsPerAgeGroup, mon::statKey().withAge(ag).withCohort(cs).withSpecies(i).withGenotype(0), eir);
    ...
    else if (simulationMode == forcedEIR)
    {
        ...
        // mon::record(mon::measure::innoculationsPerAgeGroup, mon::statKey().withAge(ag).withCohort(cs).withGenotype(0), eir);
        // EIR.assign(1, eir);
    ...
    else
    {
        ...
        for (size_t i = 0; i < speciesIndex.size(); ++i)
        {
            ...
            for (size_t g = 0; g < EIR_i.size(); ++g)
            {
                ...
                mon::record(mon::measure::innoculationsPerAgeGroup, mon::statKey().withAge(ag).withCohort(cs).withSpecies(i).withGenotype(g), eir_i + eir_l);
                EIR_i[g] += eir_i;
                EIR_l[g] += eir_l;
            }
        }
